...
| | 能力 |
|---|---|
| 🪟 **桌面浮窗** | 与主界面**实时双向同步**（改完待办立刻反映到浮窗）；无边框玻璃卡片，**就在桌面上**（默认不抢占最前，可在设置里「固定在桌面最前」）。只显示**今天**：今日课程 + 全部作业 DDL 汇总；**课程与作业都能直接勾选完成**（作业勾完自动归档）；点击课程卡片展开教师/地点/该课作业与截止时间；点「打开完整课表」进入主界面。记住位置与大小，支持**开机自启（开机只显示浮窗）**。固定深色，也可让它跟随主界面主题 |
...
| 🎯 **Todo List（醒目区）** | 主界面顶部右侧面板，**默认折叠**成与「今天没有课」等高的小卡片（只显示待办数量 + 下一个 DDL），点一下展开；支持**新建与二次编辑**（修改内容 / 关联课程 / 优先级 / 排期 / DDL / **备注详情**），备注会直接显示在卡片与浮窗上；4 种筛选、逾期高亮、勾选完成自动归档 |
