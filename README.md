...
| | 能力 |
|---|---|
| 🪟 **桌面浮窗** | 无边框 Mica 玻璃卡片，**就在桌面上**（默认不抢占最前，可在设置里「固定在桌面最前」）。只显示**今天**：今日课程 + 全部作业 DDL 汇总；**课程与作业都能直接勾选完成**（作业勾完自动归档）；点击课程卡片展开教师/地点/该课作业与截止时间；点「打开完整课表」进入主界面。记住位置与大小，支持**开机自启（开机只显示浮窗）**。固定深色，也可让它跟随主界面主题 |
| 🪟 **原生材质** | 浮窗使用 Windows 11 的 **Mica** 亚克力材质 + 半透明卡片（0.72 不透明度），真正透出桌面背景 |
| ➕ **手动添加课程** | 设置里一键「添加课程」：名称 / 教师 / 地点 / 学分 / 11 色配色 / **逐周勾选周次（含单双周）** / 在 7×13 网格上按住拖动选时段。创建后进入「**未排课时段**」托盘，**拖到课表任意位置自动吸附**完成排课 |
...
| 🌗 **浅色 / 深色主题** | **默认深色**，可在设置里改为浅色或跟随系统；整套颜色走 CSS 变量。浮窗默认固定深色，也可开启「跟随主界面主题」 |
